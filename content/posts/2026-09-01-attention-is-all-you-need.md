...
### QK^T는 quadratic form이 아니다

강의자료에서 "$QK^\top$는 quadratic form"이라는 표현을 봤는데, **엄밀히는 부정확합니다.**

$QK^\top = XW_QW_K^\top X^\top = XMX^\top$이고 성분은 $(QK^\top)_{ij}=x_i M x_j^\top$입니다.

- **대각 성분** ($i=j$): 같은 벡터를 두 번 쓰므로 진짜 **quadratic form**
- **비대각 성분** ($i \neq j$): 서로 다른 두 벡터라서 **bilinear form**

다만 $X$ 전체에 대한 함수로 보면 $X$가 정확히 두 번 곱해지므로 **차수 2 동차다항식**이 맞습니다.
"$X$에 대해 이차"라는 의미로는 정확한 표현이에요.

그리고 **비선형성 논증은 이 구분과 무관하게 성립합니다.** $X$를 2배 하면 결과가 $2^2 = 4$배가
되므로 $f(ax)=af(x)$가 깨집니다. $Q=XW_Q$와 $K=XW_K$는 각각 선형이지만 **곱한 $QK^\top$은
비선형**이고, 여기에 softmax 비선형성이 추가됩니다.
...
1. **"더하면 정보가 섞인다"** — 직관은 맞지만 고차원 준직교성 때문에 실제로는 문제가 되지
   않습니다
2. **"$QK^\top$는 quadratic form"** — 비대각 성분은 bilinear form입니다
3. **sinusoidal의 회전행렬 성질이 상대 위치를 "보장"한다고 읽음** — $M$이 회전과 commute해야만
   성립하는 조건부 성질입니다
4. **단일 시드 실험 결과를 일반화할 뻔함** — gradient 코사인 유사도를 300 시드로 다시 재니
   결론이 바뀌었습니다

3번과 4번이 특히 기억에 남습니다. 둘 다 **"그럴듯한 설명"에서 멈추지 않고 한 번 더 따져봤을 때만**
드러나는 것들이었거든요.
